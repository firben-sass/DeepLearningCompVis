...
from datasets import FrameVideoDataset, OpticalFlowDataset
...
def load_frame_dataset(batch_size=64, image_size=224):
    ...
    # Train 
    trainset = FrameVideoDataset(root_dir=DATASET_ROOT_DIR, split='train', transform=transform, stack_frames=True)
    train_loader = DataLoader(trainset,  batch_size=batch_size, shuffle=False)
    # Val
    valset = FrameVideoDataset(root_dir=DATASET_ROOT_DIR, split='val', transform=transform, stack_frames=True)
    val_loader = DataLoader(valset,  batch_size=batch_size, shuffle=False)
    # Test
    testset = FrameVideoDataset(root_dir=DATASET_ROOT_DIR, split='test', transform=transform, stack_frames=True)
...
def main():
    dual_stream_model = check_models()
    ...
    # --- Chech dataset sizes ---
    logging.info(f"Video frame train dataset size: {len(frame_trainset)}")
    ...
    optimizer = torch.optim.Adam(dual_stream_model.parameters(), lr=1e-4)
